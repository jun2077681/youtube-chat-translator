...
// In M7 session mode, the system prompt lives in the native host (claude-session.js)
// and is applied as a CLI flag at session spawn. The user message is just the JSON
// items payload — no instructions need to be repeated per request.

function buildPrompt(items) {
  const payload = { items: items.map((it) => ({ id: it.id, ja: it.ja })) };
...
async function translateBatch(items, maxTurns = 0) {
  ...
  const translations = parsed.results
    .filter((r) => r && knownIds.has(r.id) && typeof r.ko === "string")
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === MSG.TRANSLATE_KO_TO_JA) {
    ...
    sendToHost({ type: "translate", direction: "ko_to_ja", prompt, timeoutMs: 30_000 })
      .then((reply) => {
        if (!reply.ok) {
          sendResponse({ ok: false, error: reply.error || "host error" });
          return;
        }
        const parsed = parseClaudeJson(reply.text);
        const result = parsed && Array.isArray(parsed.results) && parsed.results[0];
        if (result && typeof result.ja === "string") {
          sendResponse({ ok: true, ja: result.ja, elapsedMs: reply.elapsedMs });
        } else {
          sendResponse({ ok: false, error: "no translation in response", raw: reply.text });
        }
      })
      .catch((err) => sendResponse({ ok: false, error: String(err && err.message || err) }));
    return true;
